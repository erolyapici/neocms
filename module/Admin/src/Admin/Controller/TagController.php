    ...
    public function addAction(){
        ...
        if($name === FALSE){
            ...
            $resolver->setMap(array(
                'add' => __DIR__ . '../../../../view/admin/tag/add.phtml'
            ));
            ...
            $viewModel->setTemplate('add')
                ->setVariables(array(
                    'save_url'  => $this->url()->fromRoute('tag', array('action'=>'add')),
                ));
    ...
    public function editAction(){
        ...
        if(!empty($id)){
            ...
            if(empty($post_id)){
                ...
                $resolver = new Resolver\TemplateMapResolver();
                $resolver->setMap(array(
                    'edit'=>__DIR__ . '../../../../view/admin/tag/edit.phtml'
                ));
                $renderer->setResolver($resolver);
                ...
                $viewModel->setTemplate('edit')
                    ->setVariables(array(
                        'data'=>$data,
                        'save_url' => $this->url()->fromRoute('tag', array('action'=>'edit','id'=>$id)),
                    ));
                $html = $neoAjax->strip($renderer->render($viewModel));
                $neoAjax->html('#myModal',$html);
                $neoAjax->showModal('#myModal');
